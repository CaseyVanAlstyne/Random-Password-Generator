...
var characterList = [
  typeLowerCaseLetters,
  typeUpperCaseLetters,
  typeNumbers,
...
function getUserInput() {
  ...
  if (isCharTypeNum == true) userChoices = userChoices.concat(typeNumbers);
...
function writePassword() {
  ...
  if (
    userChoices.length < typeNumbers.length ||
    (getLength < 8 && userChoices.length < typeNumbers.length) ||
    getLength > 128
  ) {
    alert(
      "Can you not count? You are supposed to pick more than 8 characters and/or fewer than 128 characters! Try again!"
    );
  } else {
    for (i = 0; i < getLength; i++) {
      newPassword +=
        userChoices[Math.floor(Math.random() * (userChoices.length - 1))];
    }
